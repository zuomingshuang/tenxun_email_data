...
def get_receive_mail():
    page = int(get_receive_page())
    for p in range(page):
        try:
            url = 'https://m.exmail.qq.com/cgi-bin/mail_list'
            data = {
            ...
            for div in divs:
                # print(div)
                name = div.find('nobr').text.strip()  #发送人
                # receive_time=div.find('span',class_='maillist_listItem_time').text.strip()
                subject=div.find('div',class_='maillist_listItemLineSecond func_ellipsis').text.strip()  #主题
                # body=div.find('div',class_='maillist_listItem_abstract func_ellipsis').text.strip()

                href='https://m.exmail.qq.com' + div.find('a')['href']
                # print(href)
                res=requests.get(url=href,cookies=cookies)
                receive_time=BeautifulSoup(res.text,'lxml').find('span',class_='readmail_item_date').text.strip()  #接收时间
                body=BeautifulSoup(res.text,'lxml').find('div',class_='readmail_item_contentNormal qmbox').text.strip()  #正文

                print(name,receive_time,subject,body)
                receive_data['发送人'].append(name)
                receive_data['接收时间'].append(receive_time)
...
def save_receive_mail():
    get_receive_mail()
    pd.DataFrame(receive_data).to_excel(user.split('@')[0]+'收件箱数据.xlsx',index=False)
